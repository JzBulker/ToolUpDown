(function() {
  ...
  function onClick(e) {
    ...
    if (e.target.hasAttribute('id')) {
      ...
      //databaseTodosGet(renderAllTodos);
      //});
      if	("Tooldown1"==e.target.getAttribute('id'))	{
	 databaseTodosAdd("Tool down", function() {
     databaseTodosGet(renderAllTodos);
     });
     }
	 else if ("Toolup1"==e.target.getAttribute('id'))	{
	 databaseTodosAdd("Tool up", function() {
     databaseTodosGet(renderAllTodos);
     });
     } 
	else {
  ...
  function todoToHtml(todo) {
	  var theDate = new Date(todo.timeStamp);
      timeString = theDate.toLocaleTimeString();
	  dateString = theDate.toLocaleDateString();
    // return '<li id="'+todo.timeStamp+'">'+todo.text+'</li>';
	return '<li id="'+todo.timeStamp+'">'+todo.text+' '+dateString+' '+timeString+'</li>';
	//return '<li id="'+todo.timeStamp+'">'+todo.text+todo.timeStamp+'</li>';
  }
